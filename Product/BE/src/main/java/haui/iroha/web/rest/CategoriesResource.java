...
/**
 * REST controller for managing {@link haui.iroha.domain.Categories}.
 */
@RestController
@RequestMapping("/api")
public class CategoriesResource {
    ...
    public ResponseEntity<CategoriesDTO> createCategories(@RequestBody CategoriesDTO categoriesDTO) throws URISyntaxException {
        ...
        if (categoriesDTO.getIdCategory() != null) {
            throw new BadRequestAlertException("A new categories cannot already have an ID", ENTITY_NAME, "idexists");
        }
        categoriesDTO.setUpdatedAt(ZonedDateTime.now());
        categoriesDTO.setCreatedAt(ZonedDateTime.now());
        CategoriesDTO result = categoriesService.save(categoriesDTO);
        return ResponseEntity
            .created(new URI("/api/categories/" + result.getIdCategory()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getIdCategory().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /categories/:idCategory} : Updates an existing categories.
     *
     * @param idCategory the id of the categoriesDTO to save.
    ...
    public ResponseEntity<CategoriesDTO> updateCategories(
        ...
        log.debug("REST request to update Categories : {}, {}", idCategory, categoriesDTO);
        if (categoriesDTO.getIdCategory() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(idCategory, categoriesDTO.getIdCategory())) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, categoriesDTO.getIdCategory().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /categories/:idCategory} : Partial updates given fields of an existing categories, field will ignore if it is null
     *
     * @param idCategory the id of the categoriesDTO to save.
     * @param categoriesDTO the categoriesDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated categoriesDTO,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/categories/{idCategory}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<CategoriesDTO> partialUpdateCategories(
        @PathVariable(value = "idCategory", required = false) final Long idCategory,
        @RequestBody CategoriesDTO categoriesDTO
    ) throws URISyntaxException {
        log.debug("REST request to partial update Categories partially : {}, {}", idCategory, categoriesDTO);
        if (categoriesDTO.getIdCategory() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(idCategory, categoriesDTO.getIdCategory())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, categoriesDTO.getIdCategory().toString())
        );
    }
